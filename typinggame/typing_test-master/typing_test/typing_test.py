...
class _Game:
    ...
    @staticmethod
    def _word_generator(args: Any) -> None:
        words = []
        # styleword= Text()
        for line in open(args.vocab):
            word = line.strip()
            if args.min_length <= len(word) <= args.max_length:
                # styleword.append(word)
                # for eachstylewords in styleword:
                #     eachstylewords = Text(eachstylewords)
                #     eachstylewords.stylize("bold magenta", 0, 6)
                words.append(word)
    ...
    def calculate_cpm(self, time_played: int):
    ...
    @staticmethod
    def _get_line(words: Any, max_chars: Any) -> None:
        line = []
        chars = 0
        for word in words:
            length = len(word)
    ...
    def _progressive_display(self, stdscr: Any, time_left: int) -> None:
        ...
        stdscr.addstr(
            "Time left: {:d}, WPM: {:d}\n".format(time_left, wpm), curses.A_BOLD
        )
    ...
    def _10ff_display(self, stdscr: Any, time_left: int) -> None:
        _height, width = stdscr.getmaxyx()
        width = min(width, MAX_WIDTH)
        stdscr.clear()
        ...
        stdscr.addstr("\n" + " ".join(self.next_line))
        stdscr.addstr("\n" + self.input, curses.A_BOLD)
        stdscr.refresh()

    def _update_display(self, stdscr: Any, time_left: int) -> None:
    ...
    @staticmethod
    def _setup_ncurses(stdscr: Any) -> None:
        # hide cursor
        curses.curs_set(0)

        # setup colors for printing text to screen
        curses.use_default_colors()
        curses.init_pair(_Game.CORRECT_COLOR, curses.COLOR_GREEN, 0)
        curses.init_pair(_Game.INCORRECT_COLOR, curses.COLOR_RED, 0)
    ...
    def _game_loop(self, stdscr: Any) -> None:
